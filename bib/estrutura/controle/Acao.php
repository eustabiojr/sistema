...
class Acao implements InterfaceAcao {
    ...
    public function __construct(Callable $acao)
    {
        ...
        if (!is_callable($this->acao)) {
            ...
            throw new Exception(NucleoTradutor::traduz('Método &1 deve receber um parâmetro do tipo &2', __METHOD__, 'Callback')). ' <br> '.
                                (NucleoTradutor::traduz('Verifique se a ação (&1) existe', $string_acao));
        }
     ...
     */
    public function obtParametrosCampo()
    {
        $campo_parametros = [];
